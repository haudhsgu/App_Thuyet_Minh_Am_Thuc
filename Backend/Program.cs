...
static async Task EnsureUserProfileColumnsAsync(Backend.Data.AppDbContext context)
{
    ...
    if (providerName.Contains("Sqlite", StringComparison.OrdinalIgnoreCase))
    {
        ...
        if (!existingColumns.Contains("FullName")) addColumnCommands.Add("ALTER TABLE Users ADD COLUMN FullName TEXT NOT NULL DEFAULT '';");
        if (!existingColumns.Contains("PhoneNumber")) addColumnCommands.Add("ALTER TABLE Users ADD COLUMN PhoneNumber TEXT NOT NULL DEFAULT '';");
        if (!existingColumns.Contains("Email")) addColumnCommands.Add("ALTER TABLE Users ADD COLUMN Email TEXT NOT NULL DEFAULT '';");
        if (!existingColumns.Contains("AvatarUrl")) addColumnCommands.Add("ALTER TABLE Users ADD COLUMN AvatarUrl TEXT NOT NULL DEFAULT '';");
    ...
    if (providerName.Contains("Npgsql", StringComparison.OrdinalIgnoreCase) || providerName.Contains("PostgreSQL", StringComparison.OrdinalIgnoreCase))
    {
        var existingColumns = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        await using var command = context.Database.GetDbConnection().CreateCommand();
        command.CommandText = @"SELECT column_name FROM information_schema.columns WHERE table_name = 'Users' OR table_name = 'users';";
        ...
        if (!existingColumns.Contains("email")) addColumnCommands.Add("ALTER TABLE \"Users\" ADD COLUMN \"Email\" text NOT NULL DEFAULT '';");
        if (!existingColumns.Contains("avatarurl")) addColumnCommands.Add("ALTER TABLE \"Users\" ADD COLUMN \"AvatarUrl\" text NOT NULL DEFAULT '';");
        if (!existingColumns.Contains("haspaid")) addColumnCommands.Add("ALTER TABLE \"Users\" ADD COLUMN \"HasPaid\" boolean NOT NULL DEFAULT false;");
...
static async Task EnsureUserBillingColumnsAsync(Backend.Data.AppDbContext context)
{
    var providerName = context.Database.ProviderName ?? string.Empty;
    ...
    if (providerName.Contains("Sqlite", StringComparison.OrdinalIgnoreCase))
    {
        ...
        var addColumnCommands = new List<string>();
        if (!existingColumns.Contains("HasPaidAccess")) addColumnCommands.Add("ALTER TABLE Users ADD COLUMN HasPaidAccess INTEGER NOT NULL DEFAULT 0;");
        if (!existingColumns.Contains("PaymentActivatedAt")) addColumnCommands.Add("ALTER TABLE Users ADD COLUMN PaymentActivatedAt TEXT NULL;");
        if (!existingColumns.Contains("HasPaid")) addColumnCommands.Add("ALTER TABLE Users ADD COLUMN HasPaid INTEGER NOT NULL DEFAULT 0;");
    ...
    if (providerName.Contains("Npgsql", StringComparison.OrdinalIgnoreCase) || providerName.Contains("PostgreSQL", StringComparison.OrdinalIgnoreCase))
    {
        ...
        var addColumnCommands = new List<string>();
        if (!existingColumns.Contains("haspaidaccess")) addColumnCommands.Add("ALTER TABLE \"Users\" ADD COLUMN \"HasPaidAccess\" boolean NOT NULL DEFAULT false;");
        if (!existingColumns.Contains("paymentactivatedat")) addColumnCommands.Add("ALTER TABLE \"Users\" ADD COLUMN \"PaymentActivatedAt\" timestamp with time zone NULL;");
