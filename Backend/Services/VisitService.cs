...
namespace Backend.Services
{
    ...
    public class VisitService : IVisitService
    {
        ...
        public async Task<List<DashboardDailyVisitDto>> GetDailyStatsAsync(Guid foodStallId, DateTime fromDate, DateTime toDate)
        {
            ...
            return queryResult.Select(item => new DashboardDailyVisitDto
            {
                VisitDate = DateTime.SpecifyKind(item.VisitDate, DateTimeKind.Utc),
                ValidVisitCount = item.ValidVisitCount
            }).ToList();
        }
